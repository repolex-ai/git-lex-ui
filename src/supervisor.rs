//! Starting and watching the per-repo `git-lex-serve sparql` processes.
//!
//! **The endpoint is `sparql`, not `viz`, and that choice is the whole
//! architecture.** `viz` is the old viewer's own backend: it serves that
//! viewer's HTML and script from `.lex/www`, walks its own port, and opens a
//! browser tab pointing at itself on startup with no flag to stop it. Feeding
//! a new interface from it meant every soul opened here also opened the old
//! page — and suppressing that took a macOS sandbox profile to deny one exec,
//! which is a lot of machinery to stop a data source from showing you a
//! different user interface.
//!
//! `sparql` has no interface attached. It answers the W3C SPARQL protocol,
//! has a real `/health`, and an `/info` that names the repo it is serving. It
//! also binds the port it is given or exits — no walking, so the port we ask
//! for is the port we get, and a failure to bind is a failure we hear about
//! rather than a server quietly listening somewhere else.
//!
//! Two things about the child still shape this module.
//!
//! **It takes no `--repo`.** The only flag is `--port`; the child resolves
//! which repository it serves from its own working directory. So the repo is
//! selected by `current_dir` on the spawn, and getting that wrong does not
//! error — it silently serves a different repo.
//!
//! **Health is asked, never remembered, and "asked" means asking *who are
//! you*.** A supervisor that recorded starting a server on 7900 and later
//! finds something answering on 7900 has learned nothing: on 2026-08-27 two
//! dev servers exited between one message and the next while the page kept
//! rendering its last data and looked entirely fine. Every check compares the
//! genesis sha the endpoint reports against the one we expected.
...
impl Supervisor {
    ...
    async fn verify(&self, port: u16, expected: Option<&str>) -> Result<(), String> {
        let client = SparqlClient::new(self.http.clone(), port);

        // Liveness first, so a dead port reports as unreachable rather than
        // as a mysterious query failure.
        if !client.health().await.map_err(|e| format!("port {port} is not answering: {e}"))? {
            return Err(format!("port {port} reports its store is not open"));
        }
        ...
        match (expected, got) {
            ...
            (Some(_), None) => Err(format!("port {port} did not report a genesis sha")),
            // No commits means no identity to check against. Accepted, and
            // said plainly rather than counted as a passed check.
            (None, _) => Ok(()),
        }
    }
...
/// `git-lex SPARQL endpoint on http://127.0.0.1:7900/sparql`
